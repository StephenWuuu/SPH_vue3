...
const tradeInfo = computed(() => {
  return store.tradeInfo;
});

onMounted(() => {
  store.getAddressInfo();
  store.getTradeInfo();
});

const submitOrder = async () => {
  let tradeNo = tradeInfo.value.tradeNo;
  let data = {
    consignee: "张三",
    consigneeTel: "15569256909",
    deliveryAddress: "广东深圳宝安",
    paymentComment: msg.value,
    orderDetailList: tradeInfo.value.detailArrayList,
  };
  let result = await store.submitOrder({ tradeNo, data });
  if (result.code == 200) {
