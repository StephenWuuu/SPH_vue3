                  ...
                  <tr>
                    <th colspan="5">
                      <span class="ordertitle"
                        >{{ order.expireTime }}　订单编号：7867473872181848
...
import { centerStore } from "@/store/center";

const page = ref(1);
const limit = ref(3);
const store = centerStore();
onMounted(() => {
  store.getOrderInfo(page.value, limit.value);
});
const orderInfo = computed(() => {
  return store.orderInfo;
});
const getPageNo = (page) => {
  store.getOrderInfo(page, limit.value);
};
</script>
